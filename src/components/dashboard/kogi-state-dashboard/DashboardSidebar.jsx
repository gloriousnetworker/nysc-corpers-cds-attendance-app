    ...
    <aside className={`hidden md:block ${darkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg ${collapsed ? 'w-16' : 'w-64'} transition-all duration-300 h-[calc(100vh-4rem)] sticky top-16`}>
      ...
      {!collapsed && (
        <div className={`p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-200'} mt-4`}>
          <div className={`text-xs font-semibold mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>KOGI INFO</div>
          <div className="mb-3 p-3 bg-gradient-to-r from-orange-50 to-amber-50 dark:from-orange-900/20 dark:to-amber-900/20 rounded-lg">
            <div className="text-xs text-[#FF8C00] dark:text-amber-300 mb-1">Kogi Zone</div>
            <div className="font-bold text-[#008753] dark:text-green-400">Zone {userData?.cdsZone || '3'}</div>
            <div className="text-xs text-orange-600 dark:text-orange-400 mt-1">{userData?.lga || 'Lokoja'} LGA</div>
          </div>
          
          <button className="w-full bg-gradient-to-r from-[#008753] to-[#FFA500] text-white py-2 px-4 rounded-lg hover:opacity-90 transition font-medium text-sm mb-2">
          ...
          <div className="mt-4 space-y-2">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-green-600 dark:text-green-400">₦4,500</span>
            </div>
            <div className="flex justify-between items-center">
